...
public class SchedulerBolt extends BaseRichBolt {
    ...
    //添加了两个变量
    private String UPSTREAM_REL_FIELDS;
    private String UPSTREAM_KEY_FIELDS;
    private OutputCollector collector;
    //private CountingBloomFilter bf;
    private CountingBloomFilter bf_R;
    private CountingBloomFilter bf_S;
    private CountMinSketch cms;
    private CountMinSketch cms_R;
    private CountMinSketch cms_S;

    //删除了参数 String UPStreamCompoentID，添加了两个变量的传参
    //public SchedulerBolt(String UPStreamCompoentID, String UPSTREAM_COMPONENT_ID, String UPSTREAM_REL_FIELDS, String UPSTREAM_KEY_FIELDS) {
    public SchedulerBolt(String UPSTREAM_COMPONENT_ID, String UPSTREAM_REL_FIELDS, String UPSTREAM_KEY_FIELDS) {
        this.UPSTREAM_COMPONENT_ID = UPSTREAM_COMPONENT_ID;
        this.UPSTREAM_REL_FIELDS = UPSTREAM_REL_FIELDS;
        this.UPSTREAM_KEY_FIELDS = UPSTREAM_KEY_FIELDS;
    }
    ...
    public void execute(Tuple tuple) {
        if (tuple.getSourceComponent().equals(UPSTREAM_COMPONENT_ID)) {
            ...
            String ts = tuple.getStringByField("Constraints.tsFileds");
            ...
            if (rel.equals("R")) {
                ...
                if (!bf_S.membershipTest(hk) && type.equals(1))
                    bf_S.add(hk);
                if (bf_S.membershipTest(hk) && type.equals(0))
                    bf_S.delete(hk);
            }
        }
        collector.ack(tuple);
